...
cum_mortality <- growth_base %>% 
  filter(Died == "1") %>% 
  select(Died, SP, Treat, Date.dead) %>%
  mutate(Date.dead = as.Date(Date.dead, format = "%d-%b-%y")) %>%
  group_by(SP) %>%
  arrange(Date.dead, .by_group = TRUE) %>%
  mutate(cum_death = cumsum(Died)) %>%
  group_by(SP, Date.dead) %>%
  filter(cum_death == max(cum_death)) %>%
  ungroup()

cum_mortality$temp_at_death = NA
for (i in 1:length(cum_mortality$Died)) {
  treat_daily_temp = all_daily_temps[which(all_daily_temps$Treat == cum_mortality$Treat[i]),]
  cum_mortality$temp_at_death[i] = treat_daily_temp$daily_avg_temp[which(treat_daily_temp$date == cum_mortality$Date.dead[i])]
}

total_snails = 84
total_snails_cedar = 83
cum_mortality$surv_proportion = (total_snails-cum_mortality$cum_death)/total_snails
cum_mortality$surv_proportion[1] = (total_snails_cedar-cum_mortality$cum_death[1])/total_snails_cedar
cum_mortality$surv_proportion[2] = (total_snails_cedar-cum_mortality$cum_death[2])/total_snails_cedar
cum_mortality$surv_proportion[3] = (total_snails_cedar-cum_mortality$cum_death[3])/total_snails_cedar

# add entries for surv proportion at start of expt
cum_mortality = rbind(cum_mortality, cum_mortality[1:4,])
cum_mortality$Date.dead[27] = as.Date("2025-06-13", format = "%Y-%m-%d")
cum_mortality$surv_proportion[27] = 1
cum_mortality$SP[28] = "Kwak"
cum_mortality$Date.dead[28] = as.Date("2025-06-13", format = "%Y-%m-%d")
cum_mortality$surv_proportion[28] = 1
cum_mortality$SP[29] = "Pruth"
cum_mortality$Date.dead[29] = as.Date("2025-06-13", format = "%Y-%m-%d")
cum_mortality$surv_proportion[29] = 1
cum_mortality$Date.dead[30] = as.Date("2025-06-13", format = "%Y-%m-%d")
cum_mortality$surv_proportion[30] = 1

# Custom colors
custom_colors <- c("Cedar" = "red", "Heron" = "blue", "Kwak" = "orange", "Pruth" = "green")

# survival curves
p = ggplot(cum_mortality, aes(x = Date.dead, y = surv_proportion, color = SP)) +
  geom_line(aes(group = SP), size = 1.5) +
  geom_point(size = 2, alpha = 0.5) +
  theme_bw(base_size = 30) +
  theme(plot.title = element_blank()) +  # Center the title
  labs(x = 'Date',
       y = 'Survival proportion',
       title = "survival curves") +
  scale_color_manual(values = custom_colors,
                     name = "Source\npopulation",
                     breaks = c("Heron", "Cedar", "Pruth", "Kwak"))
p

setwd("C:/Users/dlcyli/OneDrive/Development of thesis/Nucella experiments/GitHub")
ggsave(filename="survival_curves.png", height=8, width=15, plot=p)
