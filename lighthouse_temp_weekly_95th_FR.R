...
egg_1 <- egg[ , c(1,3)] %>% 
  ...
  filter(date > "1970-01-01" & date < "2020-01-01", 
         temp != 999.9) %>% 
  filter(md_format > "06-26" & md_format < "07-27") %>% 
  mutate(week = ifelse((md_format >= "06-27" & md_format <= "07-04"), "week3",
                       ifelse((md_format >= "07-05" & md_format <= "07-12"), "week4",
                       ifelse((md_format >= "07-13" & md_format <= "07-18"), "week5",
                       ifelse((md_format >= "07-19" & md_format <= "07-26"), "week6", NA)))))
...
setwd("C:/Users/dlcyli/OneDrive/Development of thesis/Nucella experiments/GitHub/")
saveRDS(egg_2, "CC_lighthouse_weekly_95th_temp_FR.Rds")
saveRDS(departure_2, "SoG_lighthouse_weekly_95th_temp_FR.Rds")
